...
_EXAMPLES: dict[str, str] = {
    ...
    "league:read": (
        ...
        'Filters: gameweek eq <current_gw>\n'
        'Order: rank asc, limit 30\n'
        'Note: league_id auto-injected by middleware. Do NOT filter by league_name — '
        'middleware handles league scoping automatically.\n\n'
        '### Rival squad read\n'
        'User: "show the league leader\'s squad"\n'
        'Action: db_read on squads\n'
        'Filters: manager_id eq <rival_integer_id> (from league_standings.manager_id)\n'
        'Note: For rival squads, you MUST pass manager_id explicitly as an integer. '
        'Middleware only auto-injects YOUR manager_id when no filter is present. '
        'The rival\'s manager_id is an integer from the standings data, not a UUID.\n\n'
        '### Rivalry data gathering\n'
        'User: "compare my squad with the leader"\n'
        'Step 1: db_read league_standings, gameweek eq <current_gw>, order rank asc '
        '→ identify leader\'s manager_id (integer) from rank=1 row\n'
        'Step 2: db_read squads (no manager_id filter — middleware injects yours) → your squad\n'
        'Step 3: db_read squads, manager_id eq <leader_integer_id> → rival squad\n'
        'Step 4: ANALYZE: compute_differentials between the two squads'
    ),
    "league:analyze": (
